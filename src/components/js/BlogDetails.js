...
const BlogDetails = () => {
  ...
  const fetchDetails = async() => {
    const res = await axios.get(`https://bloggify-backend.onrender.com/api/blog/${id}`).catch(err => console.log(err));
  ...
  const sendRequest = async () => {
    ...
    const res = axios.put(`https://bloggify-backend.onrender.com/api/blog/update/${id}`,{
      title: inputs.title,
